    ...
    return (
        <>
            <div className='flex items-center justify-end gap-4 lg:hidden'>
                <form action="" className='w-20 group focus-within:w-25 delay-25 transition-all'>
                    <input type="search" placeholder='Search' className='border border-blue-600 w-full placeholder:text-xs px-2 py-1 rounded-full outline-none focus:border-2 delay-25 transition-all placeholder:text-blue-600/25 placeholder:font-light' value={search} onChange={(e) => setSearch(e.target.value)} />
                </form>
                <button className='text-base text-blue-600' onClick={() => setOpen(!open)}><PiListLight /></button>
            </div>
            <div className={`fixed shadow-sm bg-white ${open ? 'top-15 opacity-100' : 'opacity-0 -top-full'} p-8 w-full grid grid-cols-1 gap-2 content-start justify-items-center text-blue-600 delay-250 transition-all z-10`}>
